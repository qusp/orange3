...
class OWLSLOutput(widget.OWWidget):
    ...
    def __init__(self):
        ...
        self.stream_type_control = gui.lineEdit(box, self, 'stream_type', 'Stream type:', orientation='horizontal', enterPlaceholder=True, callback=lambda: self.property_changed('stream_type'), tooltip="Type of output data stream. Emitted data will have this content type.")
        self.source_id_control = gui.lineEdit(box, self, 'source_id', 'Source id:', orientation='horizontal', enterPlaceholder=True, callback=lambda: self.property_changed('source_id'), tooltip="Source ID of the node. This should be a unique identifier, ideally persistent.")
        self.srate_control = gui.lineEdit(box, self, 'srate', 'Srate:', orientation='horizontal', enterPlaceholder=True, callback=lambda: self.property_changed('srate'), tooltip="Sampling rate override. Allows to override the nominal sampling rate.")
        self.chunk_len_control = gui.lineEdit(box, self, 'chunk_len', 'Chunk len:', orientation='horizontal', enterPlaceholder=True, callback=lambda: self.property_changed('chunk_len'), tooltip="Output chunk length. Number of samples in emitted chunks. 0 = same as incoming.")
        self.max_buffered_control = gui.lineEdit(box, self, 'max_buffered', 'Max buffered:', orientation='horizontal', enterPlaceholder=True, callback=lambda: self.property_changed('max_buffered'), tooltip="Preferred output buffer size. This much data will be buffered in case of network hitches.")
