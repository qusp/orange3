...
class OWSupportVectorClassification(cpewidget.CPEWidget):
    ...
    def __init__(self):
        ...
        self.kernel_control = gui.comboBox(box, self, 'kernel', label='Kernel:', items=('linear', 'poly', 'rbf', 'sigmoid'), sendSelectedValue=True, orientation='horizontal', callback=lambda: self.property_changed('kernel'), tooltip="Kernel type to use. This is a non-linear transform of the feature space, allowing for non-linear classification. Note that, instead of using 'linear' here, using the linear version of this node is usually faster.")
        self.probabilistic_control = gui.checkBox(box, self, 'probabilistic', label='Probabilistic', callback=lambda: self.property_changed('probabilistic'), tooltip="Output probabilities instead of class labels. Using probabilities results in slower learning.")
        self.tolerance_control = gui.lineEdit(box, self, 'tolerance', label='Tolerance:', orientation='horizontal', callback=lambda: self.property_changed('tolerance'), tooltip="Tolerance for stopping criterion.")
        self.max_iter_control = gui.lineEdit(box, self, 'max_iter', label='Max iter:', orientation='horizontal', callback=lambda: self.property_changed('max_iter'), tooltip="Max number of iterations. If set to -1, no limit is in effect.")
        ...
        self.class_weight_control = gui.lineEdit(box, self, 'class_weight', label='Class weight:', orientation='horizontal', callback=lambda: self.property_changed('class_weight'), tooltip="Per-class weights. If given as dict, allows to override the weights per class.")
        self.shrinking_control = gui.checkBox(box, self, 'shrinking', label='Shrinking', callback=lambda: self.property_changed('shrinking'), tooltip="Use shrinking heuristic.")
        self.cache_size_control = gui.lineEdit(box, self, 'cache_size', label='Cache size:', orientation='horizontal', callback=lambda: self.property_changed('cache_size'), tooltip="Cache size in MB.")
