...
class OWSparseBayesianRegression(cpewidget.CPEWidget):
    ...
    def __init__(self):
        ...
        super().__init__(SparseBayesianRegression())
        ...
        self.threshold_lambda_control = gui.lineEdit(box, self, 'threshold_lambda', label='Threshold lambda:', orientation='horizontal', callback=lambda: self.property_changed('threshold_lambda'), tooltip="Weight pruning threshold. Lower values can lead to somewhat faster optimization potentially at the cost of some accuracy.")
        self.alpha_shape_control = gui.lineEdit(box, self, 'alpha_shape', label='Alpha shape:', orientation='horizontal', callback=lambda: self.property_changed('alpha_shape'), tooltip="Alpha shape parameter. Shape parameter for the Gamma distribution prior over the alpha parameter.")
        self.alpha_rate_control = gui.lineEdit(box, self, 'alpha_rate', label='Alpha rate:', orientation='horizontal', callback=lambda: self.property_changed('alpha_rate'), tooltip="Alpha rate parameter. Rate parameter for the Gamma distribution prior over the alpha parameter.")
        self.lambda_shape_control = gui.lineEdit(box, self, 'lambda_shape', label='Lambda shape:', orientation='horizontal', callback=lambda: self.property_changed('lambda_shape'), tooltip="Lambda shape parameter. Shape parameter for the Gamma distribution prior over the lambda parameter.")
        self.lambda_rate_control = gui.lineEdit(box, self, 'lambda_rate', label='Lambda rate:', orientation='horizontal', callback=lambda: self.property_changed('lambda_rate'), tooltip="Lambda rate parameter. Rate parameter for the Gamma distribution prior over the lambda parameter.")
