...
class OWLASSORegression(cpewidget.CPEWidget):
    ...
    def __init__(self):
        ...
        super().__init__(LASSORegression())
        ...
        self.num_alphas_control = gui.lineEdit(box, self, 'num_alphas', label='Num alphas:', orientation='horizontal', callback=lambda: self.property_changed('num_alphas'), tooltip="Number of alpha values to fit. This determines how densely the regularization path is explored.")
        ...
        self.tolerance_control = gui.lineEdit(box, self, 'tolerance', label='Tolerance:', orientation='horizontal', callback=lambda: self.property_changed('tolerance'), tooltip="Optimization tolerance. The default stopping criterion. Can be increased to speed up computation.")
        ...
        self.min_alpha_control = gui.lineEdit(box, self, 'min_alpha', label='Min alpha:', orientation='horizontal', callback=lambda: self.property_changed('min_alpha'), tooltip="Minimum regularization strength. This is expressed as a factor of the maximum regularization strength, which is calculated from the data.")
        self.alphas_control = gui.lineEdit(box, self, 'alphas', label='Alphas:', orientation='horizontal', callback=lambda: self.property_changed('alphas'), tooltip="Override regularization strengths. Optionally the alpha values to search over can be given here (instead of the auto-generated list based on num_alphas and epsilon).")
        self.selection_control = gui.comboBox(box, self, 'selection', label='Selection:', items=['cyclic', 'random'], sendSelectedValue=True, orientation='horizontal', callback=lambda: self.property_changed('selection'), tooltip="Parameter update schedule. Random can be significantly faster for higher tol settings.")
        self.positivity_constraint_control = gui.checkBox(box, self, 'positivity_constraint', label='Positivity constraint', callback=lambda: self.property_changed('positivity_constraint'), tooltip="Constrain weights to be positive.")
        self.precompute_control = gui.lineEdit(box, self, 'precompute', label='Precompute:', orientation='horizontal', callback=lambda: self.property_changed('precompute'), tooltip="Precompute shared data. Precompute some shared data that is reused during parameter search. Aside from 'auto', can be True, False, or the actual Gram matrix.")
