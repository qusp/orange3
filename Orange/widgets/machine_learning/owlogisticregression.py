...
class OWLogisticRegression(cpewidget.CPEWidget):
    ...
    def __init__(self):
        ...
        self.regularizer_control = gui.comboBox(box, self, 'regularizer', label='Regularizer:', items=('l1', 'l2'), sendSelectedValue=True, orientation='horizontal', callback=lambda: self.property_changed('regularizer'), tooltip="Regularization term. Selecting l2 (default) gives small weights, and l1 gives sparse weights. If l1 is selected, the liblinear solver is automatically used.")
        self.alphas_control = gui.lineEdit(box, self, 'alphas', label='Alphas:', orientation='horizontal', callback=lambda: self.property_changed('alphas'), tooltip="Regularization strength. Larger values cause stronger regularization. The default is  intentionally coarse for quick turnaround -- refine it for  better results. (Note for scikit-learn users: this is the inverse of C in logreg).")
        ...
        self.solver_control = gui.comboBox(box, self, 'solver', label='Solver:', items=['newton-cg', 'lbfgs', 'liblinear'], sendSelectedValue=True, orientation='horizontal', callback=lambda: self.property_changed('solver'), tooltip="Algorithm to use. L1 is only supported with liblinear.")
        ...
        self.multiclass_control = gui.comboBox(box, self, 'multiclass', label='Multiclass:', items=['ovr', 'multinomial'], sendSelectedValue=True, orientation='horizontal', callback=lambda: self.property_changed('multiclass'), tooltip="Multi-class formulation. Multinomial is only support for lbfgs.")
