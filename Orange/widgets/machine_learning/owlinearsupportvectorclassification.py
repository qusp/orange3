...
class OWLinearSupportVectorClassification(widget.OWWidget):
    ...
    def __init__(self):
        ...
        self.loss_control = gui.lineEdit(box, self, 'loss', 'Loss:', orientation='horizontal', enterPlaceholder=True, callback=lambda: self.property_changed('loss'), tooltip="Data loss function. l1 is the hinge loss (standard SVM), l2 is the squared hinge loss.")
        self.regularizer_control = gui.lineEdit(box, self, 'regularizer', 'Regularizer:', orientation='horizontal', enterPlaceholder=True, callback=lambda: self.property_changed('regularizer'), tooltip="Regularization term. Selecting l2 (default) gives small weights, and l1 gives sparse weights.")
        ...
        self.bias_scaling_control = gui.lineEdit(box, self, 'bias_scaling', 'Bias scaling:', orientation='horizontal', enterPlaceholder=True, callback=lambda: self.property_changed('bias_scaling'), tooltip="Scale for bias term. Since this implementation applies the regularization to the bias term too, you can use this scale to counter the effect.")
