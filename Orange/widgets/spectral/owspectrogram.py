...
class OWSpectrogram(cpewidget.CPEWidget):
    ...
    def __init__(self):
        ...
        self.overlap_samples_control = gui.lineEdit(box, self, 'overlap_samples', label='Overlap samples:', orientation='horizontal', callback=lambda: self.property_changed('overlap_samples'), tooltip="Number of overlapped samples. If None, defaults to segment_samples//8.")
        ...
        self.window_control = gui.lineEdit(box, self, 'window', label='Window:', orientation='horizontal', callback=lambda: self.property_changed('window'), tooltip="Type of window function to use. Can be a string (see scipy.signal.windows.get_window) or a tuple of window name and parameter.")
        self.fft_size_control = gui.lineEdit(box, self, 'fft_size', label='Fft size:', orientation='horizontal', callback=lambda: self.property_changed('fft_size'), tooltip="Length of the FFT used, if a zero-padded FFT is desired. If none, defaults to segment_samples.")
