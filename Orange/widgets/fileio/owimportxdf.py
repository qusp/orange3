...
class OWImportXDF(cpewidget.CPEWidget):
    ...
    def __init__(self):
        ...
        self.filename_control = gui.lineEdit(box, self, 'filename', label='Filename:', orientation='horizontal', callback=lambda: self.property_changed('filename'), tooltip="File name to load (*.xdf or *.xdfz).")
        self.verbose_control = gui.checkBox(box, self, 'verbose', label='Verbose', callback=lambda: self.property_changed('verbose'), tooltip="Whether to print diagnostics.")
        self.retain_streams_control = gui.lineEdit(box, self, 'retain_streams', label='Retain streams:', orientation='horizontal', callback=lambda: self.property_changed('retain_streams'), tooltip="Streams to retain. The order doesn't actually matter (it's always data streams first, marker streams second).")
        self.handle_clock_sync_control = gui.checkBox(box, self, 'handle_clock_sync', label='Handle clock sync', callback=lambda: self.property_changed('handle_clock_sync'), tooltip="Enable clock synchronization. Needed if data were recorded across multiple computers.")
        self.handle_jitter_removal_control = gui.checkBox(box, self, 'handle_jitter_removal', label='Handle jitter removal', callback=lambda: self.property_changed('handle_jitter_removal'), tooltip="Enable jitter removal for regularlysampled streams.")
        self.handle_clock_resets_control = gui.checkBox(box, self, 'handle_clock_resets', label='Handle clock resets', callback=lambda: self.property_changed('handle_clock_resets'), tooltip="Handle clock resets. Whether the importer should check for potential resets of the clock of a stream (e.g. computer restart during recording, or hot-swap).")
