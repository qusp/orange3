...
class OWInfomaxICA(cpewidget.CPEWidget):
    ...
    def __init__(self):
        ...
        self.max_iters_control = gui.lineEdit(box, self, 'max_iters', label='Max iters:', orientation='horizontal', callback=lambda: self.property_changed('max_iters'), tooltip="Maximum number of iterations.")
        ...
        self.learning_rate_control = gui.lineEdit(box, self, 'learning_rate', label='Learning rate:', orientation='horizontal', callback=lambda: self.property_changed('learning_rate'), tooltip="Learning rate. Reduce if the algorithm diverges.")
        self.calib_seconds_control = gui.lineEdit(box, self, 'calib_seconds', label='Calib seconds:', orientation='horizontal', callback=lambda: self.property_changed('calib_seconds'), tooltip="Data length for calibration. In seconds. Note that for many channels, ICA needs quite a bit of calibration data.")
