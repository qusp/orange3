...
class OWConcentrationIndex(cpewidget.CPEWidget):
    ...
    def __init__(self):
        ...
        self.frequencies_num_control = gui.lineEdit(box, self, 'frequencies_num', label='Frequencies num:', orientation='horizontal', callback=lambda: self.property_changed('frequencies_num'), tooltip="Frequency bands to average over in the numerator. Default: Beta band.")
        self.frequencies_den_control = gui.lineEdit(box, self, 'frequencies_den', label='Frequencies den:', orientation='horizontal', callback=lambda: self.property_changed('frequencies_den'), tooltip="Frequency bands to average over in the numerator. Default: Theta and Alpha bands.")
