...
class OWStreamData(cpewidget.CPEWidget):
    ...
    def __init__(self):
        ...
        self.timing_control = gui.comboBox(box, self, 'timing', label='Timing:', items=('wallclock', 'deterministic'), sendSelectedValue=True, orientation='horizontal', callback=lambda: self.property_changed('timing'), tooltip="Timing source. Can be either 'wallclock', in which case the data is streamed at a particular factor of real time (e.g., 1x) based on the wall-clock time, or 'deterministic', in which case the data is streamed out in chunks of a particular pre-determined length (e.g., 32 samples per chunk). The deterministic mode is primarily useful for debugging.")
        self.speedup_control = gui.lineEdit(box, self, 'speedup', label='Speedup:', orientation='horizontal', callback=lambda: self.property_changed('speedup'), tooltip="Wall-clock data rate factor. This is the data rate factor relative to real time (e.g. 2 means that the data plays back at 2x real time).")
        self.update_interval_control = gui.lineEdit(box, self, 'update_interval', label='Update interval:', orientation='horizontal', callback=lambda: self.property_changed('update_interval'), tooltip="Deterministic update interval. This is the approx duration of each emitted chunk.")
        self.jitter_percent_control = gui.lineEdit(box, self, 'jitter_percent', label='Jitter percent:', orientation='horizontal', callback=lambda: self.property_changed('jitter_percent'), tooltip="Deterministic update jitter. This is the jitter, in percent, that is applied to the update interval. Corresponds to 1 standard deviation of a Gaussian distribution.")
        self.randseed_control = gui.lineEdit(box, self, 'randseed', label='Randseed:', orientation='horizontal', callback=lambda: self.property_changed('randseed'), tooltip="Deterministic random seed. This is the random seed that can be used to yield different execution traces.")
        self.hitch_probability_control = gui.lineEdit(box, self, 'hitch_probability', label='Hitch probability:', orientation='horizontal', callback=lambda: self.property_changed('hitch_probability'), tooltip="Probability of simulated hitches. In Percent. A hitch is an occasional empty packet.")
        self.looping_control = gui.checkBox(box, self, 'looping', label='Looping', callback=lambda: self.property_changed('looping'), tooltip="Looping playback. Whether to stream the data in a loop.")
