...
class OWClamp(cpewidget.CPEWidget):
    ...
    def __init__(self):
        ...
        self.min_control = gui.lineEdit(box, self, 'min', label='Min:', orientation='horizontal', callback=lambda: self.property_changed('min'), tooltip="Minimum value.")
        self.max_control = gui.lineEdit(box, self, 'max', label='Max:', orientation='horizontal', callback=lambda: self.property_changed('max'), tooltip="Maximum value.")
