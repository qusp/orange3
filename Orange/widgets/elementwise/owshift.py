...
from neuropype.nodes.elementwise import Shift


class OWShift(cpewidget.CPEWidget):

    # Node meta-data.
    name = "Shift"
    description = "Shift data by a constant offset."
    ...
    def __init__(self):
        ...
        if not [k for k, v in settings.items() if v != None]:
            super().__setattr__('offset', self.node.offset)
        ...
        self.offset_control = gui.lineEdit(box, self, 'offset', 'Offset:', orientation='horizontal', enterPlaceholder=True, callback=lambda: self.property_changed('offset'), tooltip="Shift offset.")
