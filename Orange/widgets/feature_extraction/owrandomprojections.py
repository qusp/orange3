...
class OWRandomProjections(cpewidget.CPEWidget):
    ...
    def __init__(self):
        ...
        self.num_components_control = gui.lineEdit(box, self, 'num_components', label='Num components:', orientation='horizontal', callback=lambda: self.property_changed('num_components'), tooltip="Number of components. If left to auto, will be determined from the number of observations.")
        ...
        self.epsilon_control = gui.lineEdit(box, self, 'epsilon', label='Epsilon:', orientation='horizontal', callback=lambda: self.property_changed('epsilon'), tooltip="Embedding quality. Only used when auto-deducing component number. Smaller values yield higher-quality embeddings and higher-dimensional results.")
        self.random_seed_control = gui.lineEdit(box, self, 'random_seed', label='Random seed:', orientation='horizontal', callback=lambda: self.property_changed('random_seed'), tooltip="Random seed (int or None). Different values yield different outcomes.")
