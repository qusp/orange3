...
class OWNonNegativeMatrixFactorization(cpewidget.CPEWidget):
    ...
    def __init__(self):
        ...
        self.max_iter_nls_control = gui.lineEdit(box, self, 'max_iter_nls', label='Max iter nls:', orientation='horizontal', callback=lambda: self.property_changed('max_iter_nls'), tooltip="Maximum number of iterations in NLS sub-problem.")
        self.sparseness_control = gui.comboBox(box, self, 'sparseness', label='Sparseness:', items=('data', 'components', 'none'), sendSelectedValue=True, orientation='horizontal', callback=lambda: self.property_changed('sparseness'), tooltip="Where to enforce sparsity in the model.")
        self.tolerance_control = gui.lineEdit(box, self, 'tolerance', label='Tolerance:', orientation='horizontal', callback=lambda: self.property_changed('tolerance'), tooltip="Tolerance for optimization. Serves as a stopping criterion.")
        self.beta_control = gui.lineEdit(box, self, 'beta', label='Beta:', orientation='horizontal', callback=lambda: self.property_changed('beta'), tooltip="Degree of sparsity. Only applies if sparseness is not none.")
        self.eta_control = gui.lineEdit(box, self, 'eta', label='Eta:', orientation='horizontal', callback=lambda: self.property_changed('eta'), tooltip="Degree of correctness to maintain. Applies if sparsity is not None.")
