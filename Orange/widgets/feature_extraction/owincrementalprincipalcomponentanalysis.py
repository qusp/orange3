...
class OWIncrementalPrincipalComponentAnalysis(cpewidget.CPEWidget):
    ...
    def __init__(self):
        ...
        self.whiten_control = gui.checkBox(box, self, 'whiten', label='Whiten', callback=lambda: self.property_changed('whiten'), tooltip="Normalize (whiten) outputs.")
        self.batch_size_control = gui.lineEdit(box, self, 'batch_size', label='Batch size:', orientation='horizontal', callback=lambda: self.property_changed('batch_size'), tooltip="Batch size. Optionally the number of samples to use for each mini-batch.")
