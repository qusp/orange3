...
class OWFastICA(cpewidget.CPEWidget):
    ...
    def __init__(self):
        ...
        self.algorithm_control = gui.comboBox(box, self, 'algorithm', label='Algorithm:', items=('parallel', 'deflation'), sendSelectedValue=True, orientation='horizontal', callback=lambda: self.property_changed('algorithm'), tooltip="Optimization approach. For highly noisy data the parallel approach is usually better.")
        self.whiten_control = gui.checkBox(box, self, 'whiten', label='Whiten', callback=lambda: self.property_changed('whiten'), tooltip="Pre-whiten the data. If disabled, the data must have been whitened otherwise before.")
        self.fun_control = gui.comboBox(box, self, 'fun', label='Fun:', items=('logcosh', 'exp', 'cube'), sendSelectedValue=True, orientation='horizontal', callback=lambda: self.property_changed('fun'), tooltip="Non-linearity to use. The logcosh function corresponds to Infomax ICA.")
