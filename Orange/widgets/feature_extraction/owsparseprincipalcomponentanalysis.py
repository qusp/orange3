...
class OWSparsePrincipalComponentAnalysis(cpewidget.CPEWidget):
    ...
    def __init__(self):
        ...
        self.ridge_alpha_control = gui.lineEdit(box, self, 'ridge_alpha', label='Ridge alpha:', orientation='horizontal', callback=lambda: self.property_changed('ridge_alpha'), tooltip="Shrinkage parameter to improve transform conditioning.")
        ...
        self.method_control = gui.comboBox(box, self, 'method', label='Method:', items=('lars', 'cd'), sendSelectedValue=True, orientation='horizontal', callback=lambda: self.property_changed('method'), tooltip="Method for optimization. The lars method is faster than coordinate descent if the components are sparse.")
